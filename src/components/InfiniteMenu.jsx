...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0, autoRotate = false) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #animate(deltaTime) {
    ...
    if (this.autoRotate && !this.control.isPointerDown) {
      const drift = quat.setAxisAngle(quat.create(), [0.12, 1, 0.08], 0.0018 * this.#deltaFrames);
      this.control.orientation = quat.multiply(quat.create(), drift, this.control.orientation);
      quat.normalize(this.control.orientation, this.control.orientation);
      this.control.rotationAxis = vec3.fromValues(0.12, 1, 0.08);
      this.control.rotationVelocity = 0.08;
    }
...
export default function InfiniteMenu({
  ...
  return (
    <div className={`infinite-menu-root ${className}`.trim()}>
